...
export default {
	...
	methods: {
		...
		handleFieldsChanges(fieldNewData) {
			...
			changedField.timeout;
			clearTimeout(changedField.timeout); 
			changedField.timeout = setTimeout(this.checkIfValid, 777, changedField); 
			changedField.awaitesValidation = true;
		},
		checkIfValid(field){
			field.awaitesValidation = false;
